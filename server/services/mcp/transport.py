...
class BaseMcpClient(ABC):
    ...
    @abstractmethod
    async def send_request(self, method: str, params: dict | None = None, timeout: float = 30.0) -> dict: ...
...
class SseMcpClient(BaseMcpClient):
    ...
    async def connect(self) -> McpServerConnection:
        ...
        try:
            async with self._http_client.stream("GET", self._config.url, headers=headers) as response:
                if response.status_code != 200:
                    return McpServerConnection(
                        name=self._server_name, type="failed",
                        config=ScopedMcpServerConfig(config=self._config, scope="user"),
                ...
                async for line in response.aiter_lines():
                    if line.startswith("data: "):
                        ...
                        self._message_url = data.get("uri") or data.get("endpoint")
                ...
                if not endpoint_found:
                    return McpServerConnection(
                        name=self._server_name, type="failed",
                        config=ScopedMcpServerConfig(config=self._config, scope="user"),
                        error="No message endpoint received from SSE stream",
                    )

            init_result = await self.send_request("initialize", {
                "protocolVersion": "2024-11-05",
                "capabilities": {},
                "clientInfo": {"name": "py-cc-learn", "version": "0.4.0"},
            })
            self._initialized = True
            ...
            return McpServerConnection(
                name=self._server_name, type="connected",
                config=ScopedMcpServerConfig(config=self._config, scope="user"),
                server_info=self._server_info,
                capabilities=self._capabilities,
                instructions=self._instructions,
    ...
    async def send_request(self, method, params=None, timeout=30.0):
        if not self._message_url:
            raise _JsonRpcError(-1, "Not connected")
        req = _JsonRpcRequest(method, params)
        ...
        resp = await self._http_client.post(
            self._message_url, json=req.to_dict(), headers=headers, timeout=timeout
        )
        data = resp.json()
        return _parse_json_rpc_response(data)
    ...
    async def send_notification(self, method, params=None):
        ...
        if self._access_token:
            headers["Authorization"] = f"Bearer {self._access_token}"
        await self._http_client.post(self._message_url, json=notif.to_dict(), headers=headers)
...
class HttpMcpClient(BaseMcpClient):
    def __init__(self, config: McpHTTPServerConfig, server_name: str):
        super().__init__(server_name)
        self._config = config
        self._http_client: httpx.AsyncClient | None = None
        self._access_token: str | None = None
    ...
    async def send_request(self, method, params=None, timeout=30.0):
        ...
        if self._config.headers:
            headers.update(self._config.headers)
        resp = await self._http_client.post(
            self._config.url, json=req.to_dict(), headers=headers, timeout=timeout
        )
        data = resp.json()
        return _parse_json_rpc_response(data)
    ...
    async def send_notification(self, method, params=None):
        ...
        if self._access_token:
            headers["Authorization"] = f"Bearer {self._access_token}"
        await self._http_client.post(self._config.url, json=notif.to_dict(), headers=headers)
...
async def connect_to_server(
    ...
    if isinstance(config, McpStdioServerConfig):
        ...
        client = HttpMcpClient(config, server_name)
        return await client.connect()
    else:
        return McpServerConnection(
            name=server_name,
